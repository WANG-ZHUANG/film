    ...
    export default {
        ...
        methods: {
            ...
            confirmXseat() {
                ...
                getData(process.env.BASE_URL + '/order/buyTickets?fieldId=' + params.fieldId + '&soldSeats='+params.soldSeats + '&seatsName=' + params.seatsName, 'get').then((res) => {
                // getData(process.env.BASE_URL + '/order/buyTickets', 'get', params).then((res) => {
                    if (!res) return;
                    if (res.status == 0) {
