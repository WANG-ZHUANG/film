        ...
        methods: {
            ...
            confirmOrder () {
                let timer = null,
                    ...
                    _this = this;
                getData(process.env.BASE_URL + '/order/getPayInfo', 'get', params).then((res) => {
                    if (!res) return;
                    if (res) {
                        if (res.status == 0) {
                            _this.showQrcode = true;
                            _this.qrcodeImg = process.env.IMGPRE + res.data.qRCodeAddress;
                            _this.tryNums++;
                            if (_this.qrcodeImg) {
                                clearTimeout(timer);
                                let resultParams = {
                                ...
                                timer = setTimeout(() => {
                                    getData(process.env.BASE_URL + '/order/getPayResult', 'get', resultParams).then((res1) => {
                                        if (!res1) return;
                                        if (res1 && res1.data) {
